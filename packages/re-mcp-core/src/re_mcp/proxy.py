...
"""Stdio-to-HTTP proxy for the re-mcp daemon.
...
This module is the default entry point for ``re-mcp`` (no arguments).
"""
...
from re_mcp import get_version, resolve_log_file
from re_mcp._process import IS_WINDOWS, pid_alive
from re_mcp.daemon import KEEPALIVE_INTERVAL, _state_dir, daemon_alive, read_state, remove_state
...
# Analysis on large binaries can take minutes; the read timeout must
...
def _lock_path(state_dir_name: str = "re-mcp") -> str:
    """Return the path of the daemon spawn lock file."""
    return str(_state_dir(state_dir_name) / "daemon.lock")


@contextlib.contextmanager
def _spawn_lock(state_dir_name: str = "re-mcp"):
...
def _ensure_daemon(backend: type[Backend]) -> dict:
    ...
    with _spawn_lock(state_dir_name):
        state = read_state(state_dir_name)
        if state is not None and daemon_alive(state) and _version_ok(state):
            log.debug("Reusing existing daemon (pid=%d, port=%d)", state["pid"], state["port"])
            return state
        ...
        return _spawn_daemon(backend)


def _spawn_daemon(backend: type[Backend]) -> dict:
    """Start a daemon subprocess and wait for its state file to appear."""
    info = backend.info()
    state_dir_name = info.state_dir_name
    env_prefix = info.env_prefix
    idle_timeout_env = f"{env_prefix}IDLE_TIMEOUT"
    idle_timeout_str = os.environ.get(idle_timeout_env) or os.environ.get(
        "IDA_MCP_IDLE_TIMEOUT", str(_DEFAULT_IDLE_TIMEOUT)
    )
    try:
        idle_timeout = int(idle_timeout_str)
    except ValueError:
        log.error("%s must be an integer, got %r", idle_timeout_env, idle_timeout_str)
        sys.exit(1)
    if idle_timeout < 0:
        log.error("%s must be >= 0, got %d", idle_timeout_env, idle_timeout)
        sys.exit(1)
    cmd = [
        sys.executable,
        "-m",
...
def stop(backend: type[Backend]) -> bool:
...
def main(backend: type[Backend]) -> None:
    """Proxy entry point: ensure daemon is running, then bridge stdio to it."""
    from re_mcp import configure_logging  # noqa: PLC0415

    configure_logging(label="proxy", env_prefix=backend.info().env_prefix)

    state = _ensure_daemon(backend)
