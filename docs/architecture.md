...
- `RoutingTool.run()` pops `database` from arguments and resolves the target worker. For Ghidra tools whose MCP annotation has `readOnlyHint=true`, it holds a refcounted transient session reference only around `proxy_to_worker()` and result enrichment, releasing it in `finally` on success, error, or cancellation. Other backends and non-read-only tools retain the existing implicit persistent attachment. Error handling (worker crashes, timeouts) remains contained within `proxy_to_worker()`.
- `lifespan()` shuts down all workers on exit.
- `check_attached(worker, session_id)` checks only persistent explicit-open ownership; an in-flight read never grants save or normal-close authority. It remains pass-through when session ID is `None` or the worker has no tracked sessions for backward compatibility.
- `close_for_session(worker, session_id, save, force)` atomically checks persistent ownership, detaches, and conditionally terminates under `_lock`. The last persistent owner receives `Busy` without being detached while any transient read is active.
...
- `build_database_list(include_state, caller_session_id)` returns all open databases with metadata and bounded ownership counts: persistent session SIDs, transient read SIDs, transient call refs, and their unique aggregate. `attached` reports the union for inventory only; save/close authority remains persistent-only.
...
Workers keep explicit `open_database` ownership separately from refcounted transient reads. Ghidra read-only routed tool calls acquire a transient ref immediately before worker dispatch and release it in `finally`; same-session parallel calls share one transient SID but retain separate call refs. Resource reads, IDA, and non-read-only routed tools keep their prior behavior. Inventory reports the unique union while save/normal-close checks use only explicit ownership.
...
When an MCP session disconnects, a cleanup callback registered on the session's exit stack idempotently clears its persistent ownership from all workers (via `detach_all(terminate=False)`). In-flight transient refs remain until their routed call's `finally` releases them, so an early disconnect cannot hide active work from the Busy fence or inventory. Workers are **not** terminated on disconnect — in Claude Code's multi-agent architecture all agents share one MCP session, so a session cycle would otherwise kill databases still in active use. Termination happens only via an explicit `close_database` call, `open_database(keep_open=False)`, or supervisor shutdown.
